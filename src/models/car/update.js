  ...
  static async updateSpecifications(client, carId, specs) {
    await client.query(
      ...
      SET engine_type = COALESCE($1, engine_type),
          ...
          manufacture_month = COALESCE($9, manufacture_month),
          color = COALESCE($10, color),
          body_type = COALESCE($11, body_type),
          steering_wheel = COALESCE($12, steering_wheel),
          drive_type = COALESCE($13, drive_type),
          clearance_status = COALESCE($14, clearance_status),
          has_catalyst = COALESCE($15, has_catalyst),
          airbags_count = COALESCE($16, airbags_count),
          interior_material = COALESCE($17, interior_material),
          interior_color = COALESCE($18, interior_color),
          has_hydraulics = COALESCE($19, has_hydraulics),
          has_board_computer = COALESCE($20, has_board_computer),
          has_air_conditioning = COALESCE($21, has_air_conditioning),
          has_parking_control = COALESCE($22, has_parking_control),
          has_rear_view_camera = COALESCE($23, has_rear_view_camera),
          has_electric_windows = COALESCE($24, has_electric_windows),
          has_climate_control = COALESCE($25, has_climate_control),
          has_cruise_control = COALESCE($26, has_cruise_control),
          has_start_stop = COALESCE($27, has_start_stop),
          has_sunroof = COALESCE($28, has_sunroof),
          has_seat_heating = COALESCE($29, has_seat_heating),
          has_seat_memory = COALESCE($30, has_seat_memory),
          has_abs = COALESCE($31, has_abs),
          has_traction_control = COALESCE($32, has_traction_control),
          has_central_locking = COALESCE($33, has_central_locking),
          has_alarm = COALESCE($34, has_alarm),
          has_fog_lights = COALESCE($35, has_fog_lights),
          has_navigation = COALESCE($36, has_navigation),
          has_aux = COALESCE($37, has_aux),
          has_bluetooth = COALESCE($38, has_bluetooth),
          has_multifunction_steering_wheel = COALESCE($39, has_multifunction_steering_wheel),
          has_alloy_wheels = COALESCE($40, has_alloy_wheels),
          has_spare_tire = COALESCE($41, has_spare_tire),
          is_disability_adapted = COALESCE($42, is_disability_adapted)
      WHERE id = (SELECT specification_id FROM cars WHERE id = $43)`,
      [
        specs.engine_type,
        specs.transmission,
        specs.fuel_type,
