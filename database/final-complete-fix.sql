...
-- First, let's check the valid enum values for location_type
DO $$
DECLARE
    enum_values text[];
    valid_location_type text;
BEGIN
    -- Get all enum values for location_type
    ...
    -- Display the valid enum values
    RAISE NOTICE 'Valid location_type values: %', enum_values;
    
    -- Select the first enum value as our default
    IF array_length(enum_values, 1) > 0 THEN
        valid_location_type := enum_values[1];
        RAISE NOTICE 'Using location_type value: %', valid_location_type;
    ELSE
        -- If no enum values found, check if it's a constraint instead
        RAISE NOTICE 'No enum type found for location_type, checking constraints...';
        
        -- Try to use 'georgia' based on previous memory
        valid_location_type := 'georgia';
        
        -- Check if the locations table has a CHECK constraint for location_type
        PERFORM 1
        FROM information_schema.check_constraints cc
        JOIN information_schema.constraint_column_usage ccu 
            ON cc.constraint_name = ccu.constraint_name
        WHERE ccu.table_name = 'locations' 
            AND ccu.column_name = 'location_type'
            AND cc.check_clause LIKE '%georgia%';
            
        IF FOUND THEN
            RAISE NOTICE 'Found constraint with ''georgia'' value, using it';
            valid_location_type := 'georgia';
        ELSE
            -- Check for 'transit' value in constraints
            PERFORM 1
            FROM information_schema.check_constraints cc
            JOIN information_schema.constraint_column_usage ccu 
                ON cc.constraint_name = ccu.constraint_name
            WHERE ccu.table_name = 'locations' 
                AND ccu.column_name = 'location_type'
                AND cc.check_clause LIKE '%transit%';
                
            IF FOUND THEN
                RAISE NOTICE 'Found constraint with ''transit'' value, using it';
                valid_location_type := 'transit';
            END IF;
        END IF;
    END IF;
    
    -- Insert locations with the valid location_type value
    EXECUTE format('
        INSERT INTO public.locations (id, city, location_type, is_transit, state, country)
        SELECT 1, ''თბილისი'', %L, false, NULL, NULL
        WHERE NOT EXISTS (SELECT 1 FROM public.locations WHERE id = 1)', valid_location_type);
        
    EXECUTE format('
        INSERT INTO public.locations (id, city, location_type, is_transit, state, country)
        SELECT 2, ''ბათუმი'', %L, false, NULL, NULL
        WHERE NOT EXISTS (SELECT 1 FROM public.locations WHERE id = 2)', valid_location_type);
        
    EXECUTE format('
        INSERT INTO public.locations (id, city, location_type, is_transit, state, country)
        SELECT 3, ''ქუთაისი'', %L, false, NULL, NULL
        WHERE NOT EXISTS (SELECT 1 FROM public.locations WHERE id = 3)', valid_location_type);
END $$;

-- Identify and drop the search vector trigger
DO $$
DECLARE
    trigger_name text;
BEGIN
